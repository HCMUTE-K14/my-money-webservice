...
public class UserServiceImp implements UserService {
    ...
    public long getNumberOfActiveUser() {
        ...
        try {
            return mMongoTemplate
                      .count(query(where("active").is(true)), User.class, DbConstraint.TABLE_USER);
        } catch (MongoException e) {
            throw new DatabaseException("Something wrong! Please try later");
        }
    }
    ...
    public boolean isEmailExists(String email) {
        try {
            User user = mMongoTemplate.findOne(query(where("email").is(email)), User.class,
                      DbConstraint.TABLE_USER);
    ...
    public User login(UserCredential userCredential) {
        ...
        try {
            ...
            User user = mMongoTemplate.findOne(
                      query(where("username").is(userCredential.getUsername()).and("password")
                                .is(hashPassword)),
    ...
    public boolean isApiKey(String userid, String token) {
        try {
            ...
            String realApiKey = mMongoTemplate
                      .findOne(query(where("user_id").is(userid)), User.class,
                                DbConstraint.TABLE_USER).getApikey();
    ...
    public void forgetPassword(String email) {
        try {
            ...
            messageEmail.append("<h1>").append("Your Password :").append(newPassword)
                      .append("\n\n</br>")
            ...
            mMongoTemplate.updateFirst(query(where("email").is(email)), update, User.class,
                      DbConstraint.TABLE_USER);
        } catch (MongoException e) {
            throw new DatabaseException("Something wrong! Please try later");
        }
    ...
    public void changePassword(String userid, String oldpassword, String newpassword) {
        try {
            ...
            User _user = mMongoTemplate.findOne(query(
                      where("user_id").is(userid).and("password").is(hashOldPassword)),
                      User.class, DbConstraint.TABLE_USER);
            if (_user == null) {
            ...
            mMongoTemplate.updateFirst(query(where("userid").is(userid)), update, User.class,
                      DbConstraint.TABLE_USER);
        } catch (MongoException e) {
            throw new DatabaseException("Something wrong! Please try later");
        } catch (Exception e) {
    ...
    public void changeProfile(User user) {
        try {
            ...
            mMongoTemplate.updateFirst(query(where("user_id").is(user.getUser_id())), update,
                      User.class,
                      DbConstraint.TABLE_USER);
        } catch (MongoException e) {
            throw new DatabaseException("Something wrong! Please try later");
    ...
    public User loginWithFacebook(User _user) {
        ...
        try {
            ...
            User user = mMongoTemplate.findOne(
                      query(where("username").is(_user.getUsername()).and("facebook_id")
                                .is(hashFacebook_id)), User.class,
                      DbConstraint.TABLE_USER);
            if (user != null) {
                user.setPassword(null);
                user.setApikey(null);
