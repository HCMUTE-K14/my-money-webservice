...
public class EventServiceImp implements EventService {
    ...
    public List<Event> getEvent(String userid) {
        try {
            ...
            List<Event> list = mongoTemplate
                      .find(searchQuery, Event.class, DbConstraint.TABLE_EVENT);
            return list;
    ...
    public void updateEvent(Event event) {
        try {
            ...
            query.addCriteria(Criteria.where("event_id").is(event.getEvent_id()).and("user_id")
                      .is(event.getUser_id()));
    ...
    public synchronized void syncEvent(List<Event> list) {
        ...
        for (int i = 0; i < listEventServer.size(); i++) {
            if (!list.contains(listEventServer.get(i))) {
                deleteEvent(listEventServer.get(i).getUser_id(),
                          listEventServer.get(i).getEvent_id());
            }
        }
