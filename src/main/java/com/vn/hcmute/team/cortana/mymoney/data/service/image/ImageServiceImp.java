...
public class ImageServiceImp implements ImageService {
    ...
    public List<Image> getAllImage(String userid) {
        try {
            List<Image> list = mMongoTemplate.find(query(where("user_id").is(userid)), Image.class,
                      DbConstraint.TABLE_IMAGE);
            return list;
    ...
    public void remove(String userid, String imageId) {
        try {
            ...
            mMongoTemplate
                      .findAndRemove(query(where("image_id").is(imageId).and("user_id").is(userid)),
                                Image.class,
                                DbConstraint.TABLE_IMAGE);
    ...
    public Image get(String userid, String imageId) {
        try {
            Image image = mMongoTemplate
                      .findOne(query(where("image_id").is(imageId).and("user_id").is(userid)),
                                Image.class, DbConstraint.TABLE_IMAGE);
            if (image == null) {
