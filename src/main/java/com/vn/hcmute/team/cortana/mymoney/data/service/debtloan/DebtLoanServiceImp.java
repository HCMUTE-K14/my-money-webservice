...
public class DebtLoanServiceImp implements DebtLoanService {
	...
	public void updateDebtLoan(DebtLoan debtLoan, String wallet_id) {
		try {
			...
			query.addCriteria(Criteria.where("debt_loan_id").is(debtLoan.getDebt_loan_id()).and("user_id")
					.is(debtLoan.getUser_id()).and("transaction.wallet.wallet_id").is(wallet_id));

			DebtLoan debt_loan = mongoTemplate.findOne(query, DebtLoan.class, DbConstraint.TABLE_DEBT_LOAN);
	...
	public void deleteDebtLoan(DebtLoan debtLoan) {
		try {
			...
			query.addCriteria(Criteria
					.where("trans_id").is(debtLoan.getTransaction().getTrans_id())
					.and("user_id").is(debtLoan.getUser_id()));
			mongoTemplate.findAndRemove(query, Transaction.class, DbConstraint.TABLE_TRANSACTION);
			
			query = new Query();
			query.addCriteria(Criteria
					.where("debt_loan_id").is(debtLoan.getDebt_loan_id()));
	...
	public List<DebtLoan> getDebtLoan(String wallet_id) {
		try{
			Query query = new Query();
			query.addCriteria(Criteria
					.where("transaction.wallet.wallet_id").is(wallet_id));
