...
public class CategoryServiceImp implements CategoryService {
    ...
    public void addCategory(Category category, String parentId) {
        try {
            ...
            if (TextUtil.isEmpty(parentId)) {
                ...
                query.addCriteria(
                          where("cate_id").is(parentId).and("user_id").is(category.getUser_id()));
                
                Category cate = mMongoTemplate
                          .findOne(query, Category.class, DbConstraint.TABLE_CATEGORY);
                ...
                mMongoTemplate
                          .updateFirst(query, update, Category.class, DbConstraint.TABLE_CATEGORY);
            }
    ...
    public void updateCategory(Category category, String oldParentId, String newParentId) {
        try {
            if (TextUtil.isEmpty(oldParentId) && TextUtil.isEmpty(newParentId)) {
                ...
                query.addCriteria(where("cate_id").is(category.getCate_id()).and("user_id")
                          .is(category.getUser_id()));
                ...
                long count = mMongoTemplate
                          .count(query(where("name").is(category.getName())), Category.class,
                ...
                query.addCriteria(where("cate_id").is(oldParentId).and("user_id")
                          .is(category.getUser_id()));
                ...
                if (TextUtil.isEmpty(newParentId)) {
                    ...
                    mMongoTemplate.updateFirst(query, update, Category.class,
                              DbConstraint.TABLE_CATEGORY);
                    return;
                }
                ...
                query1.addCriteria(where("cate_id").is(newParentId).and("user_id")
                          .is(category.getUser_id()));
                Category cate_parent = mMongoTemplate
                          .findOne(query1, Category.class, DbConstraint.TABLE_CATEGORY);
                if (cate_parent == null) {
                    throw new CategoryException("Category not exists");
                }
                ...
                mMongoTemplate.updateFirst(query1, update1, Category.class,
                          DbConstraint.TABLE_CATEGORY);
            }
    ...
    public boolean isExistsCategory(Category category) {
        try {
            Category cate = mMongoTemplate.findOne(
                      query(where("name").is(category.getName()).and("user_id")
                                .is(category.getUser_id())),
    ...
    public List<Category> getCategoryByUserId(String userid) {
        try {
            List<Category> categories = mMongoTemplate
                      .find(query(where("user_id").is(userid)), Category.class,
                                DbConstraint.TABLE_CATEGORY);
            if (categories == null) {
    ...
    public void initDefaultCategory(String userid) {
        ...
        try {
            jsonReader = new JsonReader(
                      new InputStreamReader(ResourceUtil.getDefaultCategory(), "UTF-8"));
    ...
    public synchronized void syncCategory(List<Category> categories) throws RuntimeException {
        try {
            ...
            for (int i = 0; i < categories.size(); i++) {
                query.addCriteria(Criteria.where("cate_id").is(categories.get(i).getCate_id())
                          .and("user_id")
                          .is(categories.get(i).getUser_id()));
                
                Category _category = mMongoTemplate
                          .findOne(query, Category.class, DbConstraint.TABLE_CATEGORY);
                if (_category == null) {
    ...
    public List<Category> getCategoryByTransactionType(String userid, String transactionType)
        ...
        try {
            List<Category> categories = mMongoTemplate.find(
                      query(where("user_id").is(userid).and("trans_type").is(transactionType)),
                      Category.class,
                      DbConstraint.TABLE_CATEGORY);
            if (categories == null) {
                LOG.info("Cannot get category custom");
    ...
    public List<Category> getCategoryByType(String userid, String type, String transType)
        ...
        try {
            List<Category> categories = mMongoTemplate.find(
                      query(where("user_id").is(userid).and("type").is(type).and("trans_type")
                                .is(transType)),
                      Category.class, DbConstraint.TABLE_CATEGORY);
            if (categories == null) {
                LOG.info("Cannot get category custom");
