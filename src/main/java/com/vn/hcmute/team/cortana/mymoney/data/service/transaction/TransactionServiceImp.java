...
public class TransactionServiceImp implements TransactionService {
    ...
    public List<Transaction> getAllTransaction(String userid) {
        try {
            List<Transaction> list = mMongoTemplate
                      .find(query(where("user_id").is(userid)), Transaction.class,
    ...
    public Transaction getTransactionById(String transactionId, String userid) {
        try {
            Transaction trans = mMongoTemplate.findOne(
                      query(where("trans_id").is(transactionId).and("user_id").is(userid)),
                      Transaction.class,
                      DbConstraint.TABLE_TRANSACTION);
            if (trans != null) {
                return trans;
    ...
    public List<Transaction> getTransactionByType(int type, String userid) {
        try {
            List<Transaction> list = mMongoTemplate
                      .find(query(where("type").is(type).and("user_id").is(userid)),
    ...
    public List<Transaction> getTransactionByTime(String startDate, String endDate, String userid) {
        try {
            ...
            query.addCriteria(
                      Criteria.where("date_created").gte(start).lte(end).and("user_id").is(userid));
            List<Transaction> list = mMongoTemplate
                      .find(query, Transaction.class, DbConstraint.TABLE_TRANSACTION);
            if (list != null) {
                return list;
            }
            throw new TransactionException("Cannot get transactions");
    ...
    @Override
    public List<Transaction> getTransactionByCategory(String categoryId, String startDate,
              String endDate,
              String userid) {
        try {
            ...
            List<Transaction> list = mMongoTemplate
                      .find(query(where("category.cate_id").is(categoryId).and("user_id")
                                          .is(userid).and("date_created").gte(start).lte(end)),
                                Transaction.class,
                                DbConstraint.TABLE_TRANSACTION);
            if (list != null) {
                return list;
    ...
    public void addTransaction(Transaction transaction) {
        try {
            ...
            if (transaction.getType().equals("income")) {
                mWalletService.takeInWallet(transaction.getWallet().getWallet_id(),
    ...
    public void updateTransaction(Transaction transaction) {
        try {
            ...
            Query query = new Query();
            query.addCriteria(
                      Criteria.where("trans_id").is(transaction.getTrans_id()).and("user_id")
                                .is(transaction.getUser_id()));
            
            Transaction trans = mMongoTemplate
                      .findOne(query, Transaction.class, DbConstraint.TABLE_TRANSACTION);
            
            if (!trans.getWallet().getWallet_id().equals(transaction.getWallet().getWallet_id())) {
                mWalletService
                          .takeInWallet(trans.getWallet().getWallet_id(), transaction.getAmount());
                mWalletService.takeOutWallet(transaction.getWallet().getWallet_id(),
                          transaction.getAmount());
            }
            ...
            mMongoTemplate.updateFirst(query, update, Transaction.class,
                      DbConstraint.TABLE_TRANSACTION);
            
            query = new Query();
            query.addCriteria(Criteria.where("transaction.trans_id").is(transaction.getTrans_id()));
            DebtLoan debtLoan = mMongoTemplate
            ...
            if (debtLoan != null) {
                ...
                mMongoTemplate
                          .updateFirst(query, update, DebtLoan.class, DbConstraint.TABLE_DEBT_LOAN);
            }
    ...
    public void removeTransaction(String transactionId, String userid) {
        try {
            Query query = new Query();
            query.addCriteria(
                      Criteria.where("trans_id").is(transactionId).and("user_id").is(userid));
            
            Transaction trans = mMongoTemplate
                      .findOne(query, Transaction.class, DbConstraint.TABLE_TRANSACTION);
            
            mMongoTemplate.findAndRemove(query, Transaction.class, DbConstraint.TABLE_TRANSACTION);
            query = new Query(
                      Criteria.where("transaction.trans_id").is(transactionId).and("user_id")
                                .is(userid));
    ...
    public List<Transaction> getTransactionByType(int type, String userid, String walletId) {
        try {
            List<Transaction> list = mMongoTemplate.find(
                      query(where("type").is(type).and("user_id").is(userid).and("wallet.wallet_id")
                                .is(walletId)),
                      Transaction.class, DbConstraint.TABLE_TRANSACTION);
            if (list != null) {
                return list;
    ...
    public List<Transaction> getTransactionByCategory(String categoryId, String userid,
        ...
        try {
            ...
            List<Transaction> list = mMongoTemplate.find(
                      query(where("category.cate_id").is(categoryId).and("user_id").is(userid)
                                .and("wallet.wallet_id")
                                .is(walletId).and("date_created").gte(start).lte(end)),
                      Transaction.class, DbConstraint.TABLE_TRANSACTION);
            if (list != null) {
                return list;
            }
            ...
            public void run() {
                try {
                    ...
                    for (int i = 0; i < transactions.size(); i++) {
                        query.addCriteria(
                                  Criteria.where("trans_id").is(transactions.get(i).getTrans_id())
    ...
    public List<Transaction> getTransactionByEvent(String userid, String eventId) {
        try {
            List<Transaction> list = mMongoTemplate.find(
                      query(where("user_id").is(userid).and("event.event_id").is(eventId)),
                      Transaction.class,
                      DbConstraint.TABLE_TRANSACTION);
            if (list != null) {
                return list;
    ...
    public List<Transaction> getTransactionByBudget(String userId, String startDate, String endDate,
        ...
        try {
            ...
            query.addCriteria(
                      Criteria.where("date_created").gte(start).lte(end).and("user_id").is(userId)
                                .and("category.cate_id").is(categoryId).and("wallet.wallet_id")
                                .is(walletId));
    ...
    public List<Transaction> getTransactionBySaving(String userId, String savingId) {
        try {
            List<Transaction> list = mMongoTemplate.find(
                      query(where("user_id").is(userId).and("saving.saving_id").is(savingId)),
                      Transaction.class,
                      DbConstraint.TABLE_TRANSACTION);
            if (list != null) {
                return list;
