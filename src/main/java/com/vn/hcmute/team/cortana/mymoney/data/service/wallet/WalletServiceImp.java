...
public class WalletServiceImp implements WalletService {
    ...
    public void createWallet(Wallet wallet) {
        try {
            ...
            query.addCriteria(
                      Criteria.where("wallet_id").is(wallet.getWallet_id()).and("user_id")
                                .is(wallet.getUser_id()));
    ...
    public void deleteWallet(String userid, String idwallet) {
        try {
            ...
            mongoTemplate.remove(query(where("wallet.wallet_id").is(idwallet)),
                      DbConstraint.TABLE_BUDGET);
            mongoTemplate.remove(query(where("wallet.wallet_id").is(idwallet)),
                      DbConstraint.TABLE_SAVING);
            mongoTemplate.remove(query(where("wallet.wallet_id").is(idwallet)),
                      DbConstraint.TABLE_EVENT);
            mongoTemplate.remove(query(where("wallet.wallet_id").is(idwallet)),
    ...
    private void addTransaction(int type, String userid, String wallet_id, String amount,
              String date_created) {
        try {
            Query query = new Query();
            query.addCriteria(Criteria.where("wallet_id").is(wallet_id));
            
            Wallet walletFrom = mongoTemplate
