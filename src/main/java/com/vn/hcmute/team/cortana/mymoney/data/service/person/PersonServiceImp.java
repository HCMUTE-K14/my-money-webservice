...
public class PersonServiceImp implements PersonService {
    ...
    public List<Person> getPersons(String userid) {
        try {
            ...
            List<Person> list = mongoTemplate
                      .find(searchQuery, Person.class, DbConstraint.TABLE_PERSON);
            return list;
    ...
    public void updatePerson(Person person) {
        try {
            Query query = new Query();
            query.addCriteria(
                      Criteria.where("person_id").is(person.getPerson_id()).and("user_id")
                                .is(person.getUser_id()));
    ...
    public synchronized void syncPerson(List<Person> persons) {
        try {
            ...
            for (int i = 0; i < persons.size(); i++) {
                query.addCriteria(Criteria.where("person_id").is(persons.get(i).getPerson_id())
                          .and("user_id")
                          .is(persons.get(i).getUser_id()));
                
                Person _person = mongoTemplate
                          .findOne(query, Person.class, DbConstraint.TABLE_PERSON);
                if (_person == null) {
